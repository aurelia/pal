 ...
 */
export function AggregateError(message: string,
                               innerError?: Error & { innerError?: Error },
                               skipIfAlreadyAggregate?: boolean): Error {
  if (innerError) {
  ...
  const e: Error & { innerError?: Error } = new Error(message);
  if (innerError) {
 ...
 */
export const FEATURE: Feature = {} as any;   // HACK: `FEATURE` actually gets initialized during bootstrap
                                             //       but for all practical purposes we consider it as `Feature`.
...
export interface Platform {
   ...
   * @param callback A callback that will receive each module id along with the module object.
   *                 Return true to end enumeration.
   */
...
export const PLATFORM: Platform = {
  noop() { /* no-op*/ },
  eachModule() { /* overriden by loader */ },
  moduleName(moduleName: string) {
    return moduleName;
  }
} as any; // HACK: `PLATFORM` actually gets initialized during bootstrap
...
export interface Dom {
   ...
   * @param markup A string containing the markup to turn into a template. Note:
   *               This string must contain the template element as well.
   ...
   * @param destination The destination element to inject the css text into.
   *                    If not specified it will default to the document.head.
   * @param prepend Indicates whether or not the styles should be prepended to the destination.
   *                By default they are appended.
 ...
 */
export const DOM: Dom = {} as any;  // HACK: `DOM` actually gets initialized during bootstrap
...
export function initializePAL(callback: (platform: Platform, feature: Feature, dom: Dom) => void): void {
  ...
  if (typeof (Object as any).getPropertyDescriptor !== 'function') {
    (Object as any).getPropertyDescriptor = (subject: object, name: string) => {
